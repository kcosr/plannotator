...
| `PLANNOTATOR_ATLAS_INDEX_PATH` | Override the Codebase Atlas SQLite index file. Relative paths resolve from the repository root; `~` is expanded. Default: `<repository>/.plannotator/atlas.sqlite3`. |
...
| `PLANNOTATOR_AST_GREP_PATH` | Override the ast-grep executable used for Explore code intelligence. Full installers manage ast-grep 0.45.0 under `<data dir>/vendor/ast-grep/0.45.0/ast-grep` (`ast-grep.exe` on Windows) by default. |
| `PLANNOTATOR_LSP_TYPESCRIPT_LANGUAGE_SERVER` | Override the `typescript-language-server` executable used by Explore for TypeScript and JavaScript semantic navigation. |
| `PLANNOTATOR_LSP_CLANGD` | Override the `clangd` executable used by Explore for C and C++ semantic navigation. |
| `PLANNOTATOR_MINIMAL` | **Read by the install scripts only**, not by the runtime binary. Set to `1` / `true` / `yes` to have `scripts/install.sh` / `install.ps1` / `install.cmd` install **only** the `plannotator` binary — skipping the sem and ast-grep sidecars, the agent-terminal runtime, and all per-agent skills, hooks, slash commands, and config. Equivalent to the `--minimal` (aliased `--binary-only`) flag; `--no-minimal` overrides it. Off by default. |
| `PLANNOTATOR_SKIP_AST_GREP_INSTALL` | **Read by the install scripts only.** Set to `1` / `true` / `yes` to skip installing the pinned ast-grep 0.45.0 sidecar used by Explore. Explore indexing then requires `ast-grep` on `PATH` or `PLANNOTATOR_AST_GREP_PATH`. Off by default. |
