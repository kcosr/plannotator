...
async function fixture(): Promise<string> {
	...
	await writeFile(join(root, "src", "setup.sh"), "#!/bin/sh\nrun_setup() {\n  printf 'ready\\n'\n}\n");
...
describe("buildAtlasSnapshot", () => {
	...
	test("builds stable nested nodes, metrics, symbols, and dependencies", async () => {
		...
		expect(snapshot.summary.files).toBe(7);
		expect(snapshot.summary.directories).toBe(1);
		expect(snapshot.rootName).toBe(root.split("/").pop()!);
		expect(snapshot.summary.languages.typescript?.files).toBe(3);
		expect(snapshot.summary.languages.python?.files).toBe(1);
		expect(snapshot.summary.languages.shell?.files).toBe(1);
		expect(snapshot.summary.languages.markdown?.files).toBe(1);
		expect(snapshot.summary.languages.text?.files).toBe(1);
		expect(snapshot.summary.skippedFiles).toBe(1);

		const sourceDirectory = snapshot.nodes.find((node) => node.id === "dir:src");
		const repositoryRoot = snapshot.nodes.find((node) => node.id === "root");
		const main = snapshot.nodes.find((node) => node.id === "file:src/main.ts");
		const math = snapshot.nodes.find((node) => node.id === "file:src/math.ts");
		const readme = snapshot.nodes.find((node) => node.id === "file:README.md");
		const lockfile = snapshot.nodes.find((node) => node.id === "file:Cargo.lock");
		const generated = snapshot.nodes.find((node) => node.id === "file:src/generated.ts");
		...
		expect(repositoryRoot?.bytes).toBe(snapshot.summary.bytes);
		expect(main?.language).toBe("typescript");
		expect(readme).toMatchObject({ language: "markdown", symbols: [] });
		expect(lockfile).toMatchObject({ language: "text", symbols: [] });
		expect(generated).toMatchObject({ language: "typescript", symbols: [], complexity: 0 });
	...
	test("enforces file and byte budgets", async () => {
		...
		expect(byBytes.summary.files).toBe(7);
		expect(byBytes.summary.symbols).toBe(0);
		expect(byBytes.nodes.find((node) => node.id === "file:README.md")).toMatchObject({
			lines: 3,
			symbols: [],
		});
	});
