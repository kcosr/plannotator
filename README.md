...
plannotator review --atlas             # Add Scope and Codebase Atlas views
```
...
| `PLANNOTATOR_LSP_CLANGD` | Override the `clangd` executable used for C and C++ definitions and references |
...
### Explore language servers

Explore builds its repository map with ast-grep, then starts a language server
on demand when you request definitions or references. Language servers are not
bundled.

For TypeScript and JavaScript, install `typescript-language-server` and
`typescript` so `typescript-language-server --stdio` is available on `PATH`.
The server uses the repository's `tsconfig.json` or `jsconfig.json` projects.

For C and C++, install `clangd`. Explore enables clangd's background index for
cross-file navigation. Real projects should provide `compile_commands.json`
(commonly generated by CMake with `CMAKE_EXPORT_COMPILE_COMMANDS=1`) or a
`compile_flags.txt`; without build flags, clangd can only use its fallback
compile command.
